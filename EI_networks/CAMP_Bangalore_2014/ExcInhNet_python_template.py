...
#        self.inC = incC         # number of incoming connections per neuron
...
#        ??????????????????????????????????
#        ? Setup the weight matrix W so that 
#        ? each neuron receives self.excC number of synaptic inputs
#        ? from excitatory neurons
#        ? and self.incC - self.excC number of synaptic inputs
#        ? from inhibitory neurons
#        ??????????????????????????????????
#
#        self.W = W
#        #self.W = scipy.sparse.csr_matrix(W)  # make sparse
#        
#    def _update_network(self,t,fired,inp=inp):
#        """ Updates the network dynamics from t->t+1 """
#         
#        # update the recurrent inputs
#        ?????????? add some code before and below
#        ?????????? to get synaptic input from
#        ?????????? those presynaptic neurons that fired
#        syn_inp = ??????????????????

...
# exc->all stdp equations
wmax = 3.               # hard bound on synaptic weight
...
alpha = 10*Hz*tau_ie*2    # target rate parameter, Vogels et al had 3Hz,
...
con_e = Synapses(Pe,neurons,stdp_eqns,pre=pre_eqns,post=post_eqns)
con_e.connect_random(sparseness=sparseness_e)
con_e.delay = taudelay
# By default wsyn will be initialized to zero, but set anyway
con_e.wsyn = 0.
con_i = Synapses(Pi,neurons,stdp_eqns_ie,pre=pre_eqns_ie,post=post_eqns_ie)
...
figure(facecolor='w')
hist(con_e.state('wsyn'),bins=10)
xlabel("exc weight factor")
ylabel("# of synapses")
title('Histogram of exc weights after '+str(simtime))

figure(facecolor='w')
hist(con_i.state('wsyn_ie'),bins=10)
xlabel("inh weight factor")
ylabel("# of synapses")
title('Histogram of inh weights after '+str(simtime))

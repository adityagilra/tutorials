...
eta = 1.#1e-2           # Learning rate

simtime = 5.0*second # Simulation time
...
# stdp exc->exc equations
wmax = 10.               # hard bound on synaptic weight
Apre_tau = 40*msecond    # STDP Apre time constant
Apost_tau = 20*msecond   # STDP Apost time constant
Apre0 = 1.0              # incr in Apre, on pre-spikes;
                         # at spike coincidence, delta w = -Apre0*eta
Apost0 = 0.5             # incr in Apost on post spike
stdp_eqns = ''' wsyn : 1
                dApre/dt=-Apre/Apre_tau : 1
                dApost/dt=-Apost/Apost_tau : 1
            '''
pre_eqns = 'Apre+=Apre0; wsyn+=-Apost*eta; wsyn=clip(wsyn,0,wmax); v+=wsyn*J'
post_eqns = 'Apost+=Apost0; wsyn+=Apre*eta; wsyn=clip(wsyn,0,wmax)'

# stdp inh-exc equations
tau_ie = 20*ms            # inh-exc stdp time constant
alpha = 5*Hz*tau_ie*2    # target rate parameter, Vogels et al had 3Hz,
                          # but I'm using Ostojic 2014
wmax_ie = 10.            # maximum inhibitory weight

stdp_eqns_ie = '''
wsyn_ie : 1
dA_pre_ie/dt=-A_pre_ie/tau_ie : 1
dA_post_ie/dt=-A_post_ie/tau_ie : 1
'''
pre_eqns_ie = '''A_pre_ie+=1.; wsyn_ie+=(A_post_ie-alpha)*eta;
                 wsyn_ie=clip(wsyn_ie,0,wmax_ie); v_post+=-wsyn_ie*g*J'''
post_eqns_ie = '''A_post_ie+=1.; wsyn_ie+=A_pre_ie*eta;
                wsyn_ie=clip(wsyn_ie,0,wmax_ie);'''

# Connecting the synapses
# Follow Dale's law -- exc (inh) neurons only have +ve (-ve) synapses.
sparseness_e = fC*C/float(NE) # fC*C neurons out of NE
                              # are connected to any 1 postsynaptic neuron
sparseness_i = (1-fC)*C/float(NI) # similar reasoning as above

## plasticity on all exc->exc synapses
#con_ee = Synapses(Pe,Pe,stdp_eqns,\
#    pre=pre_eqns,post=post_eqns)
# no plasticity on exc->exc synapses
con_ee = Synapses(Pe,Pe,'',pre='v_post+=J')
# Note that exactly C presynaptic connections are not made for each neuron,
# since this is probabilistic
# Need to generate oneself if one wants exactly C presynaptic neurons.
con_ee.connect_random(sparseness=sparseness_e)
## Very important: set the weights only after connecting i.e. line above!
#con_ee.wsyn = 1. # initial weight
con_ee.delay = taudelay
# No plasticity on exc->inh synapses
con_ei = Synapses(Pe,Pi,'',pre='v_post+=J')
con_ei.connect_random(sparseness=sparseness_e)
con_ei.delay = taudelay
# Plasticity on inh->exc synapses
con_ie = Synapses(Pi,Pe,stdp_eqns_ie,\
    pre=pre_eqns_ie,post=post_eqns_ie)
con_ie.connect_random(sparseness=sparseness_i)
con_ie.delay = taudelay
con_ie.wsyn_ie = 1.0
# No plasticity on inh->inh synapses
con_ii = Synapses(Pi,Pi,'',pre='v_post+=-g*J')
con_ii.connect_random(sparseness=sparseness_i)
...
# Plasticity: final weights histogram
figure()
hist(con_ie.state('wsyn_ie'),bins=10)
xlabel("synaptic weight factor")
ylabel("# of synapses")
title('Histogram of weights after '+str(simtime))

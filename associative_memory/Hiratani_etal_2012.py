# -*- coding: utf-8 -*-
"""
Associative Memory in Strong Sparse Weak Dense (SSWD) network
 from Hiratani, Teramae, and Fukai, 2012.
...
# Hopfield params
a = 0.1                         # sparseness i.e.
                                #  the fraction of active neurons per pattern

# Hopfield patterns and weights
# To do: make the patterns orthogonal
#Npatterns = N/10                # capacity = 0.13*N, so within capacity
Npatterns = 1
patterns = []
...
pattern_on_idx = 0
pattern_on = patterns[pattern_on_idx]
pattern_neurons = (pattern_on==1)
Narray = array(range(NE))
stim_pattern_fraction = 0.9
if stim_pattern_fraction != 1.0:
    stim_neurons = pattern_neurons & \
        (uniform(size=(NE,))<stim_pattern_fraction)
else: stim_neurons = pattern_neurons
on_idxs = Narray[stim_neurons]
# incomplete representation neurons,
# those from pattern_neurons that are not in stim_neurons
on_incompl_idxs = Narray[stim_neurons ^ pattern_neurons]
off_idxs = Narray[~pattern_neurons]

cue = zeros(NE)
cue[on_idxs] = pattern_on[on_idxs]

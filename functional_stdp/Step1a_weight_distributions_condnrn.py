...
vt = -54. * mV #firing threshold
vr = -60. * mV #reset potential
El = -74. * mV #leak potential
taue = 5. * ms #synaptic rise time
Ee = 0. * mV #reversal potential excitatory
...
P = NeuronGroup(1,'''dv/dt = (ge * (Ee-v) + El - v) / taum : volt
					dge/dt = -ge / taue : 1''',
...
con = Synapses(Pinp1,P,stdp_eqns,\
                on_pre='ge+=w*0.22;'+pre_eqns,on_post=post_eqns,
                method='euler')
con.connect(True)
...
# voltage monitor
sm_rho = StateMonitor(P,'v',record=[0])

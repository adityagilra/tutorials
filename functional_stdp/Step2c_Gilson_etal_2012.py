...
stdp_eqns = ''' w : 1
            ...
            '''
w0 = 0.04           # reference weight
Apre0 = 1.          # incr in Apre (LTP), on pre-spikes;
...
pre_eqns = 'Apre+=Apre0; w = clip(w - eta*Apost*log(1+w/w0*alpha)/log(1+alpha), 0,inf)'
post_eqns = 'Apost+=Apost0; w = clip(w + eta*Apre*exp(-w/w0/beta), 0,inf)'
winit = w0/2.       # initial weights are from 0 to winit
